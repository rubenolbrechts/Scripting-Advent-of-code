...
for move in moves:
        move = move.split(" ")
        number = -int(move[1])
        popped = stacks[int(move[3])][number:]
        stacks[int(move[3])] = stacks[int(move[3])][:number]
        for pop in popped:
            stacks[int(move[5])] = stacks.get(int(move[5]), stacks[int(move[5])].append(pop))
...
#MAKKELIJK ELEMENTEN VAN LIST VERWIJDEREN
list1 = ['Q','N', 'B', 'G', 'L', 'S', 'P', 'H'] 
list1 = list1[:4]
print(list1)
